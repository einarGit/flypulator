#ifndef _PROPULSION_PLUGIN_HEXA_HH_
#define _PROPULSION_PLUGIN_HEXA_HH_
...
class PropulsionPluginHexa : public ModelPlugin
{
...
public:
  ...
  {
    ROS_INFO_STREAM("Loading PropulsionPluginHexa ...");
    ...
    this->joint1 = _model->GetJoint("rotor_0_joint");
    this->joint2 = _model->GetJoint("rotor_1_joint");
    this->joint3 = _model->GetJoint("rotor_2_joint");
    this->joint4 = _model->GetJoint("rotor_3_joint");
    this->joint5 = _model->GetJoint("rotor_4_joint");
    this->joint6 = _model->GetJoint("rotor_5_joint");
    ...
    /* } */
    for (int i = 0; i<6; i++){
        rotor_link_ptr[i] = _model->GetChildLink(std::string("rotor_") + std::to_string(i));
    }
    ...
    ros::SubscribeOptions s1 =
        ros::SubscribeOptions::create<geometry_msgs::Vector3>(
            "/drone/wind_cmd", 100, boost::bind(&PropulsionPluginHexa::OnRosWindMsg, this, _1),
    ...
    ros::SubscribeOptions s2 =
        ros::SubscribeOptions::create<gazebo_msgs::LinkStates>(
            "/gazebo/link_states", 100, boost::bind(&PropulsionPluginHexa::OnlinkMsg, this, _1),
            ros::VoidPtr(), &this->rosQueue);
    ...
    ros::SubscribeOptions s3 =
        ros::SubscribeOptions::create<flypulator_common_msgs::RotorVelStamped>(
            "/drone/rotor_cmd", 100, boost::bind(&PropulsionPluginHexa::OnControlMsg, this, _1),
            ros::VoidPtr(), &this->rosQueue);
    this->rosSubControl = this->rosNode->subscribe(s3);

    // Spin up the queue helper thread.
    this->rosQueueThread = std::thread(std::bind(&PropulsionPluginHexa::QueueThread, this));

    //update world to apply constant force
    this->updateConnection = event::Events::ConnectWorldUpdateBegin(
        boost::bind(&PropulsionPluginHexa::OnUpdate, this, _1));

    ROS_INFO_STREAM("PropulsionPluginHexa Loaded !");
  }
...
/*    for (int i = 0; i<6; i++){ */
 /*        wrench_msg_tmp.header.frame_id = std::string("rotor_") + std::to_string(i); */
...
GZ_REGISTER_MODEL_PLUGIN(PropulsionPluginHexa)
} // namespace gazebo

#endif /*_PROPULSION_PLUGIN_HEXA_HH_*/
